...
## 🚨 다음 세션 첫 작업 — 5/14 회장 결정 ⭐⭐⭐ (최우선)

**회장 의도 (5/14 본질 통찰)**:
> "스윙/단기/중기/장기 각각 다른 점수 계산 / 필터 조건 / 매수·매도 룰 — 4개 알고리즘 신설 필요. 응급처치 X, 원인 알고 고쳐야지."

**현재 상태 — 응급 처치 인정**:
- 5/13에 *카드 4개 분리*만 함 (스윙/단기/중기/장기)
- 그러나 *알고리즘은 1개* (analyze_market_stock 종합 점수)
- *진짜 4트랙 알고리즘 X*. 카드 라벨만 분리한 겉모습.

**6/1 복귀 후 진행할 진짜 작업 (Phase 2 추천부)**:
1. `_calc_swing_score(stock)` — 1~5일 (모멘텀 추격)
2. `_calc_short_term_score(stock)` — 1~3주 (모멘텀+안정성)
3. `_calc_mid_term_score(stock)` — 1~3개월 (펀더멘털+모멘텀)
4. `_calc_long_term_score(stock)` — 3개월+ (가치투자 헌법)
5. 각 트랙별 *점수 + 필터 + 매수/매도 룰*
6. 백테스트 4회 (각 트랙별 검증)

**5/14 발견 잔여 사고 (Phase 2에서 통합 해결)**:
- KRX API 장애 5/13 16:00 (get_market_cap_by_ticker JSON 파싱 오류 7회) → market_scan_cache.json 생성 X → 단기/중기 카드 빈 채
- card_html period 라벨 잘못 (스윙 카드에 "장기 1년+" / "단기 1~4주" 표시)

**다음 세션 시작점**:
- 회장 "이어서 하자" 또는 "Phase 2 시작" → 4트랙 진짜 알고리즘 신설 진행
- 회장 부재 5/17~5/31 동안: *현재 시스템*만 작동 (스윙 카드만 작동, 단기/중기는 빈 채)
- 6/1 복귀 후: 진짜 작업 본격 진행
