...
## 🚀 다음 세션 시작 시 가장 먼저 확인 (6/16 최신)

### 6/16 라이브 반영 (자동매매 스윙계좌, 이미 작동)
- **방어형 분산**: 종목당 200만→**100만 / 슬롯 5→10** (같은 1,000만 2배분산=갭하락 데미지 반감). regime 슬롯 강세10/주의8/경계6 — **시장상황 조정 유지**(약세장 축소·위험장 정지). 백테스트: 슬롯10 수익↑·MDD↓ 검증.
- **버그 2개 수정**: ① 사전알림 후 조용히 매수 안 되던 문제(regime 가드를 알림 *전*으로 이동 → 알림=실제매수 일치) `0e7c010` ② 검증부 shadow 로그(verify_log.json) 워크플로우 커밋 누락 → 데이터 휘발 막음.
- **성적표 리셋**: 과거 거래이력/자산곡선/AI로그 0으로(새 룰 새 학습). 원본 백업=저장소밖 `_reset_backup_20260616`. 보유 2종목은 보존.

### 진행 중 — 이벤트 스윙(급등주 잡기) [[메모리 project_event_swing]]
- 회장 "SK오션플랜트·태웅 +30%(해상풍력 테마) 왜 놓침" → 봇이 못 잡는 이유: ①풀(283종목) 밖 ②봇 스윙=모멘텀이라 반등주 거름.
- 1단계(반등신호 백테스트, BT_REVERSAL 토글) = 표본부족(6개월 3건, 큐레이션풀에 반등 드뭄). **다음=2단계 풀 확대(거래량 급증 종목 스캔)가 선행.**
- 원칙: 단계별 백테스트 검증 후 배포(슈퍼트렌드 기각·돌파 보류 선례). 가치주 X, 스윙계좌만.

### 기각/보류된 실험 (재시도 불필요)
- 슈퍼트렌드: 기각(1~5일 스윙에 일봉추세 안 맞음). 1일보유: 기각(비용+승자조기매도). 대형주 돌파B: 보류(추세장만 약효).

🔖 **마지막 커밋**: `2cd7ff1` (반등신호 백테스트 토글). git clean.
