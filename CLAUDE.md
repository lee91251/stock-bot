...
## 🎯 5/7~ 진행 작업 순서 (사용자 결정, 5/7 KST)

> **이 섹션이 다음 세션 시작 시 가장 먼저 볼 작업 순서입니다.**

### ✅ 5/7 오늘 진행 (이번 세션)
1. **encrypt_dashboard.sh 재시도 로직** (5분) — staticrypt 일시 장애 시 재시도 → 안전망 강화
2. **트레이딩 일기 AI + 학습 연결** (3h) — 매도 후 AI가 1줄 회고 → 거래 이력에 누적 → 다음 매도 시 참조 (학습)

### 📅 다음 세션 진행 (5/8~)
3. **수익률 vs 코스피 비교 차트** (2h) — 봇이 코스피보다 잘 하는지 시각화
4. **AI 매도 어드바이저 v2 인프라** (2h) — B1 결과 신뢰도 검증 후 자동 결정 반영
   - Safe-by-default: 30건 미만 → 의견만 (현재 B1) / 30건+ → AI "보류" 권장 시 매도 1회 미루기
5. **B4 백테스트 자동 가중치 튜닝 인프라** (5h) — 자가학습 완성
   - Safe-by-default: 30건 미만 → default 임계(65) / 30건+ → 자동 조정 권장 알림 → 사용자 OK 시 적용

### ⏳ 데이터 누적 후 자동 활성화 (인프라 미리, 동작은 데이터 후)
- B1 신뢰도 검증 (1주일 운영 → 5/13쯤): #4 활성화 가능
- 매매 30건+ 누적 (2~3주 → 5/27쯤): #5 자동 가중치 활성화
