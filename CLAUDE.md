...
## 🚀 다음 세션 시작 시 가장 먼저 확인 (7/3 최신)

> **스윙 자동매매 조건 전체는 `files/SWING_RULES.md` 먼저 읽기** (코드 grep 대신). [[reference_swing_rules]]

### 이벤트 스윙(급등주 잡기) — 검증 완료, 시장폭 배포 [[project_event_swing]]
- 회장 "SK오션플랜트·태웅 +30% 왜 놓침" → 4단계 검증 결과 **시장폭 풀확대만 채택**: 좋은 장엔 종목 넓혀 벌고, 나쁜 장엔 큐레이션으로 좁혀 지킴. 3구간 백테스트 합격(연환산 현재의 3배 + 나쁜장 안 잃음).
- **반등신호·지수레짐·DART 호재공시매수·무조건 풀확대 = 전부 기각**(백테스트 손실). "지수↑ 개별종목↓" 구간엔 시장폭이 정답(지수 레짐은 실패).
- **1-A 배포됨**: 시장폭(전종목 상승비율) 매일 계산 + 텔레그램 shadow (`market_breadth_history.json`, `compute_and_save_breadth`). **1-B 대기**: 5일 관찰 후 실제 매수 적용(풀 시총상위~400 + 시장폭 게이트, `BREADTH_GATE_ENFORCE=true`).

### 6/17 영구차단 — 예고-후-미체결 [[incident_prealert_no_buy]]
- 한화오션 "30초후 매수" 예고 갔는데 미체결(HL만도/하림 재발). 원인=예고(selected)와 실제 매수 가드 불일치(**일일 금액한도**가 selected에 누락). 수정=selected에 금액한도 예측 + 미체결 종목 이유 텔레그램 (`69bec92`).

### 6/19 부서 현황판 + 스윙 룰 문서
- 대시보드 상단에 9부서 상태등 현황판 (`_make_department_board`, `4131716`).
- **스윙 조건 전체 = `files/SWING_RULES.md`** (`ea3c65f`). 코드 바뀌면 이 문서도 같이 갱신(drift 방지).

### 방어형 분산 (6/16, 유지 중)
- 종목당 200만→**100만 / 슬롯 5→10** (같은 1,000만 2배 분산 = 갭하락 데미지 반감). regime 슬롯 강세10/주의8/경계6. 성적표 리셋(백업=저장소밖 `_reset_backup_20260616`).

### 기각/보류된 실험 (재시도 불필요)
- 슈퍼트렌드 / 1일보유 / 과매도 반등신호 / 지수 레짐 / DART 호재공시 매수 / 대형주 돌파B — 전부 백테스트 손실 or 무효.

🔖 **마지막 커밋**: `ea3c65f` (SWING_RULES.md 문서화). git clean.

---

## (이전) 다음 세션 가장 먼저 확인 (6/2)
...
## 🏛️ Phase 2 14부서 분리 (5/14 회장 확정 / 5/29 복귀 완료 ⭐)

**원칙**: 4트랙 알고리즘이 *직원의 일하는 방식*을 정한 것 → 이제 *직원을 부서로 나누는 작업*.

**일정 (갱신)**:
- 5/17~5/29 회장 부재 (12일 — 부재 기간 봇 자동 운영 + 데이터 누적 완료)
- 5/29 복귀 후 우선 작업: ① 유연 보유 룰 + ② DART 검증 → 그 후 Phase 2
- 회장님이 "Phase 2 시작"이라고 말씀하시면 즉시 진행
...
✅ **회장 부재 종료 (5/29 복귀 완료)** — 부재 기간 5/17~5/29 (12일):
- 자동매매: 매수 6건 / 매도 7건 / 승률 71% (5승 2패)
- 손익비: 1:2.89 (손절이 익절의 2.9배) — *손익비 결함 발견*
- 실현 손익: -39,220원 (자동) + -141,440원 (가치주 손매도) = -180,660원
- 워크플로우 200건 중 195건 성공 / 실패 0건
- 모든 영구 차단 fix 작동 확인 (비상정지 / KRX FDR / push retry / 캐시 컷)
- 부재 매뉴얼 [[15 - 회장 부재 매뉴얼 (5월 17-31)]] → *역사 문서*로 보존

🎯 **5/29 복귀 후 작업 진행 중**:
- ✅ 카카오게임즈 5/19 손매도 기록 정정 (`7b31290`)
- ✅ DART API PER/PBR/ROE 직접 계산 — 단기/중기/장기 트랙 작동 (`ed3b597`)
- 🔄 marketscan DART 검증 진행 중
- ⏳ 유연 보유 룰 (회장 통찰: 좋은 종목 더 보유 / 나쁜 종목 빨리 청산)
- ⏳ Phase 2 14부서 분리
...
## 7. 투자 설정 (자동매매 스윙 — 실제 코드값, 6/16 방어형)

> ⚠️ 전체 매수/매도 조건은 `files/SWING_RULES.md` 참조. 아래는 핵심값만.

| 항목 | 값 |
|---|---|
| 종목당 투자금액 | **100만원** (6/16: 200만→100만 분산) |
| 매수 임계 | 스윙점수 **65** (또는 급등 모멘텀) |
| 손절선 | **−4%** |
| 1차 익절 | **+6%** (절반 매도) |
| 2차 익절 | **+10%** (전량) |
| 보유 한도 | **5거래일** (수익 +3%↑면 10일까지 연장) |
| 하루 최대 | 강세 10종목·1,000만 / 중립 8·800만 / 약세 6·600만 (regime) |
