...
**현재 상태 (2026-05-04~5/5 KST 기준)** — 자비스급 진화 완성 ⭐⭐⭐:
- 🇰🇷 KR 전용 모드 / 자동매매 모의투자 (한국투자증권)
- 💼 가치주 10종목 (미래에셋, +967만원/+44.81%) — `HOLDINGS_JSON`
- 🤖 **자동매매**: KR_STOCKS 100개 + 시장스캔 50 → 풀 ~120개, 점수 임계 65
- ⏰ cron-job.org: 09:00~15:30 30분 간격 autobuy 14회 + 16:00 marketscan
- 🦾 **자비스 비서 시스템** (5/4 신규):
  - **Phase 1**: AI 맞춤 코칭 (보유+자금+섹터 종합) — 텔레그램 `/추천` `/진단` `/오늘`
  - **Phase 1.5**: 시장 위험 지수 0~100 (VIX/공탐/코스피/이벤트 종합) → 자동매수 강도 자동 조정
  - **Phase 2**: 자가 학습 인프라 (`analyze_trading_performance`) — 데이터 30건+ 누적 시 자동 가중치 활성화
  - **Phase 3**: 능동 비서 — 매매 임박 알림(state 추적) + 위험 등급 변화 즉시 알림
- 📊 **대시보드 자비스 v3** https://lee91251.github.io/stock-bot/
  - 🦾 AI 맞춤 비서 카드 (위험 SVG 게이지 + 마크다운→HTML)
  - 📈 자산 추이 Chart.js 라인 (가치주/자동매매/누적손익, 일별 누적)
  - 💼 가치주 row 7일 sparkline (yfinance KS/KQ 자동)
  - 📢 최근 공시 카드 (DART 30분 폴링, 보유 우선)
  - 🤖 AI 시장 판단 마크다운→HTML 변환
- 🔐 **로그인 페이지 자비스 HUD 톤**: 검정 우주 + 별빛 + nebula + 글래스카드 + cyan 코너 대칭 + 스캔 라인 (form 100% top 0%→100%, 2.5s, z-index 1)
- 📨 텔레그램 다이어트 v2: DART 공시는 **보유 종목 + ⚠️/✅ 키워드만** (그 외는 대시보드만)
- 🚫 휴장일 자동 차단 (5/5 어린이날 등)
- 🔖 마지막 커밋: `a97978b` (스캔 라인 튜닝)

**📁 새로 추가된 파일들**:
- `dashboard_cache.json` — 메모지(macro/AI/추천/sparkline/risk/disclosures), GitHub Actions runner stateless 대응 commit
- `portfolio_history.json` — 자산 일별 누적 (자산 추이 차트용)
- `dart_seen.json` — DART 공시 중복 차단
- `.github/scripts/encrypt_dashboard.sh` — staticrypt 통합 (8 job 일괄)
- `.github/scripts/inject_login_style.py` — 자비스 HUD 디자인 주입

---

**📌 5/6(수) 정상 거래일 첫 가동 확인** ⭐:
1. 🦾 AI 맞춤 비서 카드 (위험 게이지 + 코칭 텍스트)
2. 📈 자산 추이 차트 (이제 2~3점 → 추세 형성 시작)
3. 💼 가치주 row sparkline (10종목 각 7일 추세)
4. 📢 최근 공시 카드 (보유 우선 + 전체 100건)
5. 텔레그램 다이어트: 스케줄 어긋남 X, 공시 핵심만 (보유+⚠️/✅)
6. 자동매매 100개 풀 + 점수 65로 매수 시도 (14회/일)
7. 진단 로그 14회 자동 기록 → 점수 분포 + 차단 사유 패턴
8. 16:00 첫 marketscan 정상 가동 (5/7부터 풀 ~120개)

---

**🔥 다음 세션에서 할 일 — [[Obsidian Vault/03 - 로드맵.md]] § "자비스 진화 후보" 참고**

자비스 진화 v4 후보 (10가지):

### 🔴 진짜 자비스 완성
1. **AI 백테스트 + 자동 가중치 튜닝** ⭐⭐⭐ (4~5h) — 매주 일요일 자동, swing_score 자동 조정
2. **즉석 종목 분석기** ⭐⭐ (2~3h) — 대시보드 검색창 + AI 즉시 분석
3. **종목별 뉴스 요약** ⭐⭐ (2h) — 보유 종목 5건 AI 종합

### 🟡 사용자 경험
4. **수익률 vs 코스피 비교** ⭐ (1.5h) — 초과 수익 시각화
5. **알림 다이어트 v2** (2~3h) — 사용자 클릭 패턴 ML 학습
6. **음성 명령** (3~4h) — PWA 음성 인식

### 🟢 운영 효율
7. **Anthropic Prompt Caching** ⭐⭐ (1h) — AI 비용 50% ↓
8. **백테스트 자동 리포트** (1h) — 매주 일요일 텔레그램
9. **가치주 헌법 위반 체크** (1.5h) — 9-투자 전략 자동 검증

### 🔵 큰 결정
10. **실전 매매 전환** ⭐⭐⭐ (1h) — PAPER → 실계좌 (1~2주 검증 후)

---

**📚 다음 세션 첫 명령 (추천 순)**:
- "5/6 첫 가동 결과 점검해줘" — 자비스 시스템 정상 작동 확인
- 또는 "Prompt Caching 추가하자" — 빠른 가치 (1h, 비용 50% ↓)
- 또는 "AI 백테스트 + 자동 튜닝 진행" — Phase 2 자가 학습 완성 (4~5h)
- 또는 `/bot-status` — 봇 현재 상태 한눈에
